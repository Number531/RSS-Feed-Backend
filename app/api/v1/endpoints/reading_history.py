...
async def get_history(
    ...
    history_list, total = await service.get_user_history(
        user_id=current_user.id, skip=skip, limit=limit, start_date=start_date, end_date=end_date
    )
    ...
    return ReadingHistoryList(items=items, total=total, skip=skip, limit=limit)
...
async def get_recent(
    ...
    service = ReadingHistoryService(db)
    recent = await service.get_recently_read(user_id=current_user.id, days=days, limit=limit)
...
async def clear_history(
    ...
    deleted_count = await service.clear_history(user_id=current_user.id, before_date=before_date)

    message = f"Successfully cleared {deleted_count} history record(s)"
    if before_date:
        message += f" before {before_date.isoformat()}"

    return ClearHistoryResponse(deleted_count=deleted_count, message=message)
...
async def update_preferences(
    ...
    prefs = await service.update_user_preferences(
        user_id=current_user.id, **data.dict(exclude_unset=True)
    )
    return ReadingPreferencesResponse.from_orm(prefs)
