...
class CommentService(BaseService):
    ...
    def __init__(
        self, comment_repository: CommentRepository, article_repository: ArticleRepository
    ):
        """
    ...
    async def get_article_comments(
        ...
        # Log operation
        self.log_operation(
            "get_article_comments", article_id=str(article_id), page=page, page_size=page_size
        )
    ...
    async def get_comment_replies(self, parent_id: UUID) -> List[Comment]:
        ...
        """
        # Log operation
        self.log_operation("get_comment_replies", parent_id=str(parent_id))
    ...
    async def get_comment_by_id(self, comment_id: UUID) -> Comment:
        ...
        """
        self.log_operation("get_comment_by_id", comment_id=str(comment_id))
    ...
    async def update_comment(self, comment_id: UUID, user_id: UUID, content: str) -> Comment:
        """
        ...
        # Log operation
        self.log_operation("update_comment", user_id=user_id, comment_id=str(comment_id))
    ...
    async def delete_comment(self, comment_id: UUID, user_id: UUID) -> Comment:
        """
        ...
        """
        # Log operation
        self.log_operation("delete_comment", user_id=user_id, comment_id=str(comment_id))
    ...
    async def build_comment_tree(self, article_id: UUID, max_depth: int = 10) -> List[dict]:
        """
        ...
        """
        # Log operation
        self.log_operation("build_comment_tree", article_id=str(article_id), max_depth=max_depth)

        try:
            # Get top-level comments
            top_comments = await self.comment_repo.get_article_comments(
                article_id=article_id, page=1, page_size=100
            )
            ...
            for comment in top_comments:
                ...
                comment_dict["replies"] = await self._get_replies_recursive(
                    comment.id, current_depth=1, max_depth=max_depth
                )
    ...
    async def _get_replies_recursive(
        self, parent_id: UUID, current_depth: int, max_depth: int
    ) -> List[dict]:
        """
        ...
        for reply in replies:
            ...
            reply_dict["replies"] = await self._get_replies_recursive(
                reply.id, current_depth + 1, max_depth
            )
