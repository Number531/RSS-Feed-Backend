...
class UserFeedSubscriptionRepository:
    ...
    async def get_user_subscription(
        ...
        result = await self.db.execute(
            ...
            .where(
                ...
                UserFeedSubscription.feed_id == feed_id
            )
        )
        return result.scalar_one_or_none()
    ...
    async def get_user_subscriptions(
        ...
        query = select(UserFeedSubscription).options(
            joinedload(UserFeedSubscription.feed)
        ...
        # Get total count
        count_query = select(func.count()).select_from(UserFeedSubscription).where(
            UserFeedSubscription.user_id == user_id
        )
        if is_active is not None:
