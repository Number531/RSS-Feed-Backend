...
from app.utils.content_utils import clean_description, extract_first_image, sanitize_html
...
class ArticleProcessingService:
    ...
    async def process_article(self, article_data: Dict, source: RSSSource) -> Optional[Article]:
        """
    ...
    async def _get_article_by_url_hash(self, url_hash: str) -> Optional[Article]:
        ...
        Returns:
            Article or None
        """
        result = await self.db.execute(select(Article).where(Article.url_hash == url_hash))
