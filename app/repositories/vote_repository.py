...
class VoteRepository:
    ...
    async def get_user_vote(self, user_id: UUID, article_id: UUID) -> Optional[Vote]:
        ...
        query = select(Vote).where(and_(Vote.user_id == user_id, Vote.article_id == article_id))
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def create_vote(self, user_id: UUID, article_id: UUID, vote_value: int) -> Vote:
        """Create a new vote."""
        vote = Vote(user_id=user_id, article_id=article_id, vote_value=vote_value)
        self.db.add(vote)
    ...
    async def update_vote(self, vote: Vote, new_value: int) -> Vote:
    ...
    async def update_article_vote_metrics(self, article_id: UUID) -> None:
        ...
        vote_sum_query = select(func.sum(Vote.vote_value)).where(Vote.article_id == article_id)
        vote_sum = await self.db.scalar(vote_sum_query)

        # Get count of votes (total number)
        vote_count_query = select(func.count(Vote.id)).where(Vote.article_id == article_id)
    ...
    async def get_comment_vote(self, user_id: UUID, comment_id: UUID) -> Optional[Vote]:
        """Get user's vote on a comment."""
        query = select(Vote).where(and_(Vote.user_id == user_id, Vote.comment_id == comment_id))
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def create_comment_vote(self, user_id: UUID, comment_id: UUID, vote_value: int) -> Vote:
        """Create a new comment vote."""
        vote = Vote(user_id=user_id, comment_id=comment_id, vote_value=vote_value)
        self.db.add(vote)
        await self.db.commit()
        await self.db.refresh(vote)
    ...
    async def update_comment_vote(self, vote: Vote, new_value: int) -> Vote:
    ...
    async def delete_comment_vote(self, vote: Vote) -> None:
    ...
    async def update_comment_vote_metrics(self, comment_id: UUID) -> None:
        """
        ...
        """
        # Get sum of vote values (score)
        vote_sum_query = select(func.sum(Vote.vote_value)).where(Vote.comment_id == comment_id)
        vote_sum = await self.db.scalar(vote_sum_query)

        # Get count of votes (total number)
        vote_count_query = select(func.count(Vote.id)).where(Vote.comment_id == comment_id)
        vote_cnt = await self.db.scalar(vote_count_query)
