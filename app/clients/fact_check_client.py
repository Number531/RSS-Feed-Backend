...
class FactCheckAPIClient:
    ...
    async def submit_fact_check(
        ...
        try:
            ...
            logger.error(
                f"Network error submitting fact-check",
    ...
    async def get_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                logger.error(
                    f"Error fetching job status",
                    extra={"job_id": job_id, "error": str(e)}
                )
            raise
        except httpx.RequestError as e:
            logger.error(
                f"Network error fetching job status",
                extra={"job_id": job_id, "error": str(e)}
            )
            raise
    ...
    async def get_job_result(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                logger.error(
                    f"Error fetching job result",
                    extra={"job_id": job_id, "error": str(e)}
                )
            raise
        except httpx.RequestError as e:
            logger.error(
                f"Network error fetching job result",
                extra={"job_id": job_id, "error": str(e)}
            )
            raise
    ...
    async def cancel_job(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error cancelling job",
                extra={"job_id": job_id, "error": str(e)}
            )
            raise
        except httpx.RequestError as e:
            logger.error(
                f"Network error cancelling job",
                extra={"job_id": job_id, "error": str(e)}
            )
            raise
