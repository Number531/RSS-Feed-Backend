...
class ArticleService(BaseService):
    ...
    async def get_articles_feed(
        ...
        try:
            ...
            metadata = self.create_pagination_metadata(
                total=total, skip=skip, limit=page_size, returned_count=len(articles)
            )
    ...
    async def get_article_by_id(self, article_id: UUID, user_id: Optional[UUID] = None) -> Article:
        """
        ...
        """
        self.log_operation("get_article_by_id", user_id=user_id, article_id=str(article_id))
    ...
    async def search_articles(
        ...
        # Log operation
        self.log_operation("search_articles", query=query, page=page, page_size=page_size)
